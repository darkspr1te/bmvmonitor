...
Looking at the back of the unit Pin 4 is to your left , Pin 1 is near the edge,
Connect your TX to TX and your RX to RX , note that it's commong to connect your TX to it's RX but they have
changed the labeling to confuse you (or not confuse the avg consumer who's told connect tx to tx and rx to rx etc) 


BMV just spits out the current values every 1-s via serial port, this sketch captures that and makes it 
available via :-
nc IPADDRESS port 
example 
nc 192.168.1.112 23 
...
once captured via python netcat we will write to a rrd DB for now. historic accurecy is not really needed
so no need for fat sql setup. 
